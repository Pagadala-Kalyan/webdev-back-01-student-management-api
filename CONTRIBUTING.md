...
Thank you for your interest in contributing! This project is a **debugging challenge** — your goal is to find and fix the 5 intentional bugs hidden in the Student Management API codebase. Here's how to get started.
...
```bash
git clone https://github.com/<your-username>/webdev-back-01-student-management-api.git
cd webdev-back-01-student-management-api
```
...
```bash
git remote add upstream https://github.com/GDG-Open-Challenge/webdev-back-01-student-management-api.git
git fetch upstream
```
...
```bash
git checkout -b fix/issue-1-query-injection-vulnerability
git checkout -b fix/issue-2-email-validation
git checkout -b fix/issue-3-delete-endpoint-response
git checkout -b fix/issue-4-gpa-calculation-logic
git checkout -b fix/issue-5-enrollment-capacity-check
```
...
5. **Test your fix** by verifying the API behavior with cURL or Postman
...
curl -X POST http://localhost:3000/api/students \
  ...
  -d '{"firstName":"John","lastName":"Doe","email":"john@example.com","studentId":"S001"}'
```

Refer to [README.md](README.md) for the complete API endpoint documentation.
...
```bash
git commit -m "fix(security): prevent noSQL injection in student search (#1)"
git commit -m "fix(validation): add email format validation to Student model (#2)"
git commit -m "fix(api): add response to DELETE student endpoint (#3)"
git commit -m "fix(calculation): correct GPA calculation logic (#4)"
git commit -m "fix(enrollment): validate course capacity before enrolling student (#5)"
```

> [!NOTE]
> Scope should be the affected area: `security`, `validation`, `api`, `calculation`, `enrollment`, etc.
...
- Set the base repository to `GDG-Open-Challenge/webdev-back-01-student-management-api` and base branch to `main`
...
| **Category**      | Bug / Security / Data Quality / API          |
| **File(s)**       | `<file path>`                                |
...
JavaScript behavior, validation logic, or data handling error that causes the issue. 
This is the most important section — demonstrate that you understand the underlying cause,
...
- [ ] Verified all CRUD operations for affected endpoints
- [ ] Tested edge cases (empty values, invalid input, boundary conditions)

**Test Evidence:**
<Describe what you tested and what the results were. Include cURL commands or
Postman request examples showing the fix works.>
...
| **No additional frameworks** | Use only existing tech stack: Node.js/Express, MongoDB, Mongoose |
...
| **Test before submitting** | Verify your fix works with cURL/Postman and doesn't break other endpoints |
